...
def demonstrate_modal(ack, shortcut, client, logger):
    ...
    client.views_open(
        ...
        view={
            "type": "modal",
            "callback_id": "new_puzzle",
            "title": {
                "type": "plain_text",
                "text": "Set up a new puzzle",
                "emoji": True,
            },
            ...
            "blocks": [
                {
                    "type": "input",
                    "block_id": "puzzle_name",
                    "element": {
                        "type": "plain_text_input",
                        "max_length": 80,
                        "action_id": "puzzle_name_input-action",
                    },
                    "label": {
                        "type": "plain_text",
                        "text": "puzzle name",
                ...
                },
                {
                    ...
                    "element": {
                        "type": "url_text_input",
                        "action_id": "puzzle_url_input-action",
                    },
                    "label": {
                        "type": "plain_text",
                        "text": "puzzle URL",
...
def create_puzzle(ack, client, logger, say, command):
    ...
    say("I have finished attempting to create this puzzle.")


@app.view("new_puzzle")
def handle_submission(ack, body, client, view, logger, say):
    ack()
    puzzle_name = view["state"]["values"]["puzzle_name"]["puzzle_name_input-action"][
        "value"
    ]
    logger.info(puzzle_name)
    draft = Puzzle()
    draft.create(client, logger, say, puzzle_name)
